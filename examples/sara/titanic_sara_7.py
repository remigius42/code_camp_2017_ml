...
def train_bagging_svm(x_train, y_train, x_cv, y_cv):
    clf = BaggingClassifier(SVC(probability=True), max_samples=0.5, max_features=0.5)
    clf.fit(x_train, y_train)
...
def predict_test(clf):
    ...
    df.to_csv('/vagrant/examples/sara/titanic-solution-iteration-7.csv', index=False)
...
clf = train_bagging_svm(x_train, y_train, x_cv, y_cv)
