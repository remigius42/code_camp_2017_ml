...
#    data['Cabin_Prefix'] = data['Cabin'].apply(lambda x: x.str[0].fillna('Z'))
...
n_estimations_list = np.linspace(1.,100.,20)
max_features_list = np.linspace(2.,30.,10)
AUC_all_estimators = []
Accuracy_all_estimators = []
AUC_all_max_features = []
Accuracy_all_max_features = []
...
for estimator_param in n_estimations_list:
    ...
    print('AUC for n_estimator ', estimator_param, ': ', auc)
    AUC_all_estimators.append(auc)
    Accuracy_all_estimators.append(accuracy)


for max_feature in max_features_list:
    max_feature = int(max_feature)
    # Random forest
    clf = RandomForestClassifier(n_estimators=10, max_features= max_feature, n_jobs=-1, random_state=0)
    # *********************************
    #  Evaluate Model
    # *********************************
    ...
    fpr, tpr, thr = roc_curve(y_test, preds[:,1])
    auc = roc_auc_score(y_test, preds[:,1])
    print('AUC for max_feature ', max_feature, ': ', auc)
    AUC_all_max_features.append(auc)
    Accuracy_all_max_features.append(accuracy)

plt.plot(n_estimations_list, AUC_all_estimators)
plt.plot(n_estimations_list, Accuracy_all_estimators)
plt.xlabel("n_estimations")
plt.show()

plt.plot(max_features_list, AUC_all_max_features)
plt.plot(max_features_list, Accuracy_all_max_features)
plt.xlabel("max_features")
plt.show()


# *********************************
#  Evaluate Model with good parameters
# *********************************
clf = RandomForestClassifier(n_estimators=35, max_features=16, n_jobs=-1)
...
df.to_csv('/vagrant/examples/remigius-thoemel/titanic-solution-iteration-3.csv',index=False)
