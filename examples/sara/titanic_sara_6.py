...
def train_bagging_random_forest(x_train, y_train, x_cv, y_cv):
    clf = BaggingClassifier(RandomForestClassifier(n_estimators=100, n_jobs=-1), max_samples=0.5, max_features=0.5)
    clf.fit(x_train, y_train)

    preds = clf.predict_proba(x_cv)
    fpr, tpr, thr = roc_curve(y_cv, preds[:, 1])
    auc = roc_auc_score(y_cv, preds[:, 1])
    print('AUC for bagging with random forest: ', auc)

    plot_curve(fpr, tpr, 'Bagging with Random Forest ' + str(auc))
    return clf
...
def predict_test(clf):
    ...
    df.to_csv('/vagrant/examples/sara/titanic-solution-iteration-6.csv', index=False)
...
#clf = train_random_forest(x_train, y_train, x_cv, y_cv)

clf = train_bagging_random_forest(x_train, y_train, x_cv, y_cv)
